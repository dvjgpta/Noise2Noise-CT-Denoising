...
def progress_bar(batch_idx, num_batches, report_interval, train_loss):
    ...
    print('\rBatch {:>{dec}d} [{}{}] Train loss: {:>1.5f}'.format(
        batch_idx + 1, '=' * fill + '>', ' ' * (bar_size - fill), train_loss, dec=str(dec)), end='')
...
def show_on_epoch_end(epoch_time, valid_time, valid_loss, valid_psnr):
    ...
    clear_line()
    print('Train time: {} | Valid time: {} | Valid loss: {:>1.5f} | Avg PSNR: {:.2f} dB'.format(
        epoch_time, valid_time, valid_loss, valid_psnr))
...
def create_montage(img_name, noise_type, save_path, source_t, denoised_t, clean_t, show):
    ...
    denoised.save(os.path.join(
        save_path, f'{fname}-{noise_type}-denoised.png'))
    fig.savefig(os.path.join(
        save_path, f'{fname}-{noise_type}-montage.png'), bbox_inches='tight')
